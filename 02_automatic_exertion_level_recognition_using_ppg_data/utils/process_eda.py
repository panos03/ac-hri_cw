...
def get_eda_measures_batch(datapath, sample_rate=64.0):
    """Extract one EDA feature row per full recording."""
    csv_fpath = os.path.join(datapath, "EDA_features.csv")
    header = [
        "sub_id", "label", "status",
    ...
    print("Found", len(eda_files), "EDA files under", datapath)
    ...
    for sub_id, fn, filepath in eda_files:
        ...
        if raw_eda.size == 0:
            ...
            feats = {k: np.nan for k in header[3:]}
        else:
            feats = extract_eda_features_window(raw_eda, sample_rate=sample_rate)
            status = "ok" if np.isfinite(feats["eda_variance"]) else "failed_extract"

        vals = [sub_id, label, status] + [feats[k] for k in header[3:]]
        with open(csv_fpath, "a+", newline="") as csvfile:
            fp_writer = csv.writer(csvfile, delimiter=",")
            fp_writer.writerow(vals)

    print("Wrote EDA features to", csv_fpath)
    return csv_fpath

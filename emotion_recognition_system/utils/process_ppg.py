...
def get_ppg_signal_filepaths(datapath):
	...
	for path, dirs, files in os.walk(datapath):
		...
		for fn in sorted(files):
			...
			# full_labelled export format from data_collection_and_label_synch.
...
def load_PPG_signal(filepath):
	# Try the full_labelled tabular format first: rec_id,label,EDA,PPG
	try:
...
def get_ppg_measures_windowed(datapath, window_sec=30, stride_sec=15, sample_rate=64.0, output_csv=None):
	...
	csv_fpath = output_csv if output_csv is not None else os.path.join(datapath, 'PPG_features_windowed.csv')
	header = [
